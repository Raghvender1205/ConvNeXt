"""
IsoTropic ConvNeXt Model Architecture
"""
...
class ConvNeXtIsotropic(nn.Module):
    ...
    """
    def __init__(self, in_chans=3, num_classes=1000, depth=18, 
                 dim=384, drop_path_rate=0., layer_scale_init_value=0, head_init_scale=1.):
        super().__init__()
    ...
    def forward_features(self, x):
        x = self.stem(x)
        x = self.blocks(x)
        return self.norm(x.mean([-2, -1])) # GlobalAveragePooling, (N, C, H, W) -> (N, C)
...
@register_model
def convertnext_isotropic_small(pretrained=False, **kwargs):
